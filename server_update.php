...
$sql="SELECT * FROM MESSAGES WHERE sent_to='$_POST[friend_id]' AND sent_from= '$_SESSION[QWERTY]'
	UNION
	SELECT * FROM MESSAGES WHERE sent_to='$_SESSION[QWERTY]' AND sent_from='$_POST[friend_id]' ORDER BY id desc, send_time desc limit 10 ";
	...
	if($result)
	{
		...
		{
			$ret.='<tr id="'.$row["id"].'"><td>'.$row['content'].'</td><td>'.$row['send_time'].'</td></tr>';
		}
		$ret.='</table>';
		echo $ret;
		//flush();
